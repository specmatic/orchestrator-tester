...
class TriggerWorkflowContractTest(unittest.TestCase):
    ...
    def test_workflow_replicates_enterprise_os_scoped_orchestrator_gates(self) -> None:
        ...
        self.assertIn("-f \"client_payload[enterprise_options][configuration]=${configuration}\"", workflow)
        self.assertIn("-f \"client_payload[enterprise_options][repository]=${GITHUB_REPOSITORY}\"", workflow)
        self.assertIn("repository_label=\"${repository_name^}\"", workflow)
        self.assertIn("-f \"client_payload[enterprise_options][repository_name]=${repository_label}\"", workflow)
        self.assertIn("-f \"client_payload[enterprise_options][run_number]=${GITHUB_RUN_NUMBER}\"", workflow)
        self.assertIn("-f \"client_payload[orchestrator_options][run_parallel]=${RUN_PARALLEL}\"", workflow)
        self.assertIn("-f \"client_payload[orchestrator_options][jar_url]=${jar_url}\"", workflow)
        self.assertIn("-f \"client_payload[orchestrator_options][test_executor_path]=${test_executor_path}\"", workflow)
        ...
        self.assertIn('payload["client_payload"]["orchestrator_options"]["jar_url"] = jar_url', workflow)
        self.assertIn('payload["client_payload"]["orchestrator_options"]["test_executor_path"] = test_executor_path', workflow)
